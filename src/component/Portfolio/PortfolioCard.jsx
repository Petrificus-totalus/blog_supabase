...
const PortfolioCard = ({ item, index, scrollYProgress }) => {
  ...
  // 变换映射
  const x = useTransform(scrollYProgress, [start, peakStart, peakEnd, end], ['120vw', '0vw', '0vw', '-120vw']);
  const scale = useTransform(scrollYProgress, [start, peakStart, peakEnd, end], [0.4, 1, 1, 0.4]);
  const opacity = useTransform(scrollYProgress, [start, peakStart - 0.05, peakEnd + 0.05, end], [0, 1, 1, 0]);
  const rotateY = useTransform(scrollYProgress, [start, peakStart, peakEnd, end], [-45, 0, 0, 45]);
  const z = useTransform(scrollYProgress, [start, peakStart, peakEnd, end], [-800, 0, 0, -800]);
  const blur = useTransform(scrollYProgress, [start, peakStart, peakEnd, end], ['10px', '0px', '0px', '10px']);
  ...
  return (
    ...
    >
      <div className="portfolio-card">
        ...
        {/* 背景光晕 */}
        <div className={`portfolio-card-glow ${getGradientClass(item.color)}`} />
        ...
        <div className="portfolio-card-content">
          <div className="portfolio-card-header">
            <div className="portfolio-card-label">
               ...
               <span className="portfolio-card-label-text">COLLECTION {item.id}</span>
            </div>
          ...
          >
            VIEW PROJECT
            <svg className="portfolio-card-button-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="portfolio-card-visual">
          ...
          >
            ...
            <img src={item.webImage} alt={item.title} className="portfolio-card-web-image" />
          </motion.div>
          ...
          >
            <img src={item.mobileImage} alt={item.title} className="portfolio-card-mobile-image" />
